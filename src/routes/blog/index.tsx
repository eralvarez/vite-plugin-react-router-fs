  ...
  return (
    <div data-testid="page-blog-index">
      ...
      <ul className="space-y-4">
        {posts.map((post) => (
          <li key={post.slug} className="rounded-lg border border-slate-700 bg-slate-800 p-4">
            <Link to={`/blog/${post.slug}`} className="text-lg font-medium text-blue-400 hover:underline">
              {post.title}
            </Link>
